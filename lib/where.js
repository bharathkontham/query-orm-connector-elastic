...
const buildWhere = function (where) {
  const self = this;

  let nestedFields = R.map(
    (val, key) => (val.type === 'nested' ? key : null),
    Object.keys(self.dataSource.settings.mappings.properties)
  );
  nestedFields = R.filter((v) => v !== null, nestedFields);
