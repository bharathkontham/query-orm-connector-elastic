...
const addDefaults = function (functionName) {
  ...
  if (self.restAsInt.indexOf(functionName) !== -1) {
    payload.rest_total_hits_as_int = true;
  }
  return payload;
};
...
const buildFilter = function (filter = {}) {
  ...
  const payload = self.addDefaults('find');
  payload.body = {};

  if (filter.limit !== undefined && filter.limit !== null) {
    payload.size = filter.limit;
  } else {
    payload.size = self.dataSource.defaultLimit || 1000;
  }
